...
void QuasiCrystalsSidebar::ViewOptions()
{
    if (ImGui::CollapsingHeader("View Options")) {
        ...
        if (auto quacry = dynamic_cast<Quasicrystal22 *>(scene->ActiveQuasiCrystal())) {
            ...
            static ImVec4 color_window = ImVec4(data->color_window_[0], data->color_window_[1], data->color_window_[2],
                                                data->color_window_[3]);
        ...
        if (auto quasi = scene->ActiveQuasiCrystal()) {
            ChangeSampleSize(*(quasi->GetSampleSize()));
            if (ImGui::Button("Resample")) quasi->Resample();
        }
    }
}
}